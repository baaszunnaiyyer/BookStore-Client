...
import { useNavigate } from "react-router-dom"; // Added proper import
...
const CreateBooks = () => {
  ...
  const handleSaveBook = () => { // Corrected spelling
    const data = {
      title,
      author,
      publishYear
    ...
    axios.post('http://localhost:5555/books', data)
      ...
      .catch((error) => { // Added error parameter
        console.log(data);
        setLoading(false);
        enqueueSnackbar("Book was not Created", {variant:'error'})
  ...
  return (
    <div className="p-4">
      ...
      <div className="flex flex-col border-2 border-sky-400 rounded-xl w-[600px] p-4 mx-auto">
        ...
        <button className="p-2 bg-sky-300 m-8" onClick={handleSaveBook}> Save </button> {/* Corrected function name */}
      </div>
    </div>
  );
}
